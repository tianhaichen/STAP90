module MaterialData
	...
	type Material
		integer:: MatType, EosType		! Type of material and equation of state
		real(8):: Density, Young, Poisson
		real(8):: WaveSpeed				! Wave speed c
		real(8):: Yield0, TangMod		! Yield stress and tangent modulus
		real(8):: B_jnc, n_jnc, C_jnc	! constants for equation of state
	end type Material
	...
	real(8):: C1eosg, C2eosg, C3eosg, C4eosg ! Gruneisen EOS parameter
...
!
	private etype_, young_, poisson_, yield0_, tangmod_, den0_, epeff_, sig_y_
	private sm, sd, sig, G2, K3, PlaMod, seqv

	integer:: etype_			! Type of equation of state
...
! ---------------------------------------------------------------------
	use Simulation, only: EngKinetic, EngStrain, DTk
	implicit none
...
!	pla1: elastic-perfectly plastic
	case(2) 
		call M3D2(de, den, mu, EngKinetic)

!	pla2: isotropic hardening
!	Ref: LS-DYNA Theoretical Manual: Material Model 10
	case(3) 
		call M3D3(de, den, mu, EngKinetic)

!	john: Johnson-Cook plasticity model
!	Ref: LS-DYNA Theoretical Manual: Material Model 15 (G.R.Johnson 1988)
	case(4) 
		call M3D4(de, den, mu, EngKinetic, mat, DTk)
...
! -                                                                   -
! -  Purpose                                                          -
! -     Rotate stess if Jaumman stress rate is used                   -
! -     Similar to DYNA3D                                             -
! -                                                                   -
! -  Input                                                            -
! -     vort - Spin * DTk                                             -
...
! ---------------------------------------------------------------------
	...
	subroutine eos_gruneisen(mu, Eng, rho, sm)
! ---------------------------------------------------------------------
! -                                                                   -
! -  Purpose                                                          -
! -     Update pressure using Mie-Gruneisen equation of state         -
! -                                                                   -
! -  Input                                                            -
! -     mu  - (Current density/Initial density - 1)                   -
! -     Eng - Kinetic energy                                          -
! -     rho - Current density                                         -
! -                                                                   -
! -  Output                                                           -
...
! ---------------------------------------------------------------------
	...
	real(8), intent(in) :: mu, Eng, rho
	real(8), intent(inout) :: sm

	sm = (C1eosg*mu + C2eosg*mu**2 + C3eosg*mu**3) * (1 - C4eosg*mu*0.5d0/rho) + C4eosg*Eng
	sm = - sm

	end subroutine eos_gruneisen
...
! ---------------------------------------------------------------------
	...
	subroutine M3D2(de, den, mu, EngKinetic)
! ---------------------------------------------------------------------
! -  Purpose                                                          -
...
! ---------------------------------------------------------------------
	...
	real(8), intent(in) :: de(6), den, mu, EngKinetic

	real(8) ratio, depeff

	call elastic_devi(de, sd, G2)
	if(etype_.eq.1) then
		call eos_gruneisen(mu, EngKinetic, den, sm)
	else
		call elastic_p(de, sm, K3)
	end if
	...
	subroutine M3D3(de, den, mu, EngKinetic)
! ---------------------------------------------------------------------
! -  Purpose                                                          -
...
! ---------------------------------------------------------------------
	...
	subroutine M3D4(de, den, mu, EngKinetic, mat, DTk)
! ---------------------------------------------------------------------
! -  Purpose                                                          -
...
! ---------------------------------------------------------------------
	...
	real(8), intent(in) :: de(6), den, mu, EngKinetic, DTk
	type(Material), intent(in) :: mat
